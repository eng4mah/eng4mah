...
def apply_custom_css():
    ...
    <style>
        ...
        /* Container for file selector popup */
        .file-selector-popup {
            background-color: #1F2937; /* Gray-800 */
            border: 1px solid #4B5563; /* Gray-600 */
            border-radius: 12px;
        ...
        .file-selector-popup .stCheckbox {
            border-bottom: 1px solid #4B5563;
            padding: 8px 0;
        }
        .file-selector-popup .stCheckbox:last-child {
            border-bottom: none;
        }
        ...
        .stButton>button:focus {
            box-shadow: 0 0 0 2px #3B82F6 !important; /* Blue-500 focus ring */
            outline: none;
        }
    </style>
    """
...
def main():
    ...
    if st.session_state.show_file_selector:
        with st.container():
            st.markdown('<div class="file-selector-popup">', unsafe_allow_html=True)
    ...
    cols = st.columns(8)
    with cols[0]:
        if st.button("📎", help="Upload a temporary file for one-time use"):
            st.session_state.show_temp_uploader = not st.session_state.show_temp_uploader
            st.session_state.show_file_selector = False
            st.session_state.action = None
            st.rerun()
    with cols[1]:
        if st.button("🗂️", help="Select which permanent files to use for context"):
            st.session_state.show_file_selector = not st.session_state.show_file_selector
            st.session_state.show_temp_uploader = False
            st.session_state.action = None
            st.rerun()
    with cols[2]:
        if st.button("📄", help="Summarize a single file"):
            st.session_state.action = "summarize_single"
            st.session_state.show_file_selector = False
            st.session_state.show_temp_uploader = False
            st.rerun()
    with cols[3]:
        if st.button("🔄", help="Compare two documents"):
            st.session_state.action = "compare_docs"
            st.session_state.show_file_selector = False
            st.session_state.show_temp_uploader = False
            st.rerun()
    with cols[4]:
        if st.button("⚠️", help="Check selected documents for contradictions"):
            st.session_state.action = "check_contradictions"
            st.session_state.show_file_selector = False
            st.session_state.show_temp_uploader = False
            st.rerun()
    with cols[5]:
        if st.button("📖", help="Define a legal term from the documents"):
            st.session_state.action = "define_term"
            st.session_state.show_file_selector = False
            st.session_state.show_temp_uploader = False
            st.rerun()
    with cols[6]:
        if st.button("✍️", help="Draft a legal clause based on the documents"):
            st.session_state.action = "draft_clause"
            st.session_state.show_file_selector = False
            st.session_state.show_temp_uploader = False
            st.rerun()
    with cols[7]:
        if st.button("🧹", help="Clear chat history and start a new topic"):
            st.session_state.messages = []
            st.session_state.action = None
            st.session_state.show_file_selector = False
            st.session_state.show_temp_uploader = False
    ...
    # --- Chat Input ---
    if prompt := st.chat_input("Ask a question or type '1994' to toggle Admin Panel..."):
        ...
        # Handle regular chat prompt
            ...
            # Gather context for the chat message
            selected_docs = [doc for doc in st.session_state.loaded_docs if doc.metadata['source'] != RULES_FILE_NAME and st.session_state.selected_docs_map.get(doc.metadata['source'])]
            all_context_docs = selected_docs
            ...
            if not all_context_docs:
                st.warning("Please select at least one document (🗂️) or upload a temporary file (📎) to ask a question.")
                st.stop()
