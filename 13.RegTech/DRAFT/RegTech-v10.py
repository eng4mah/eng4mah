...
# --- New Feature Flags ---
USE_AUTO_SUMMARIZATION = True # NEW: Master switch for the new summary feature
USE_SUMMARY_FOR_SELECTION = True # NEW: Use summaries to improve document selection
...
def get_deepseek_response_blocking(prompt: str, api_key: str) -> Optional[str]:
    """NEW: Gets a single, non-streaming response from the API."""
    headers = {
...
def generate_and_save_summary(file_name: str, doc_folder: str, summary_folder: str, api_key: str):
    """NEW: Generates and saves a 4-line summary for a document if it doesn't exist."""
    ...
    summary_prompt = f"""
أنت مساعد متخصص في تلخيص المستندات القانونية.
لخص النص التالي في 4 إلى 6 أسطر بالضبط. يجب أن يكون الملخص دقيقاً ومباشراً ويغطي النقاط الأساسية في المستند.
...
def load_all_summaries(summary_folder: str) -> str:
    """NEW: Loads all summaries from the summary folder into a single string."""
    all_summaries_text = ""
...
def get_document_selection_with_summaries(question: str, doc_names: List[str], rules: str, summaries: str, api_key: str) -> Optional[List[int]]:
    """MODIFIED: Selects documents based on rules, summaries, and the user question."""
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
    doc_list_str = "\n".join([f"#{i}: {name}" for i, name in enumerate(doc_names)])

    selection_prompt = f"""
مهمتك هي تحديد أنسب الملفات للإجابة على سؤال المستخدم. اعتمد على القواعد الإلزامية وملخصات الملفات المتوفرة.
يجب أن ترد فقط بأرقام الملفات من قائمة "الملفات المتوفرة" بصيغة: '#N, #M'.
إذا لم يكن أي ملف مناسباً، اكتب '#NONE'.
...
الملفات المتوفرة (اختر الأرقام من هنا):
---
{doc_list_str}
---
"""
...
def main():
    ...
    with chat_container:
        # --- MODIFIED: Initial Loading and Auto-Summarization ---
        ...
        # --- Action UI: Uploader (MODIFIED to trigger summarization) ---
        if st.session_state.action == "upload_doc":
            ...
            if uploaded_file is not None:
                ...
                # NEW: Generate summary for the newly uploaded file
        ...
        # --- Action UI: Summarizer (Unchanged) ---
        if st.session_state.action == "summarize_doc":
            # (This section remains unchanged as it provides on-demand summarization)
        ...
        # A. Handle new summary request (Unchanged)
        ...
        # B. Handle new question from chat input (MODIFIED with new selection logic)
        if prompt := st.chat_input("اسأل سؤالاً عن مستنداتك...", disabled=(st.session_state.action is not None)):
            ...
            with st.chat_message("assistant"):
                ...
                if USE_TWO_STEP_SELECTION and st.session_state.doc_names:
                    with st.spinner("جاري تحليل سؤالك وتحديد الوثائق المناسبة..."):
                        ...
                        if USE_SUMMARY_FOR_SELECTION:
                            ...
                            # Original selection method (kept for compatibility)
                ...
                # Step 2: Generate Final Answer (Unchanged logic, uses output from Step 1)
...
if __name__ == "__main__":
    # This function is defined for completeness but get_document_selection_with_summaries is used
    def get_document_selection(question: str, doc_names: List[str], api_key: str) -> Optional[List[int]]:
        pass # This is the old function, now superseded by the new one with summaries.
    main()
