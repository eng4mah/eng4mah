...
MAX_DOCS_FOR_SELECTION = 1 # Kept for clarity, but logic now enforces selecting one.
...
def generate_and_save_summary(file_name: str, doc_folder: str, summary_folder: str, api_key: str):
    ...
    # MODIFIED: New prompt for generating a single, connected paragraph summary.
    summary_prompt = f"""
...
"""
    ...
    if summary_text:
        try:
            ...
            # Save the summary with the filename as a header
...
def get_document_selection_with_summaries(question: str, doc_names: List[str], rules: str, summaries: str, api_key: str) -> Optional[List[int]]:
    """MODIFIED: Selects exactly one document based on rules, summaries, and the user question."""
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
    doc_list_str = "\n".join([f"#{i}: {name}" for i, name in enumerate(doc_names)])

    # MODIFIED: New prompt to enforce selecting only one document.
    selection_prompt = f"""
مهمتك هي تحديد **ملف واحد فقط** هو الأنسب للإجابة على سؤال المستخدم.
يجب أن تختار **ملفًا واحدًا فقط** من قائمة "الملفات المتوفرة".
اعتمد في اختيارك على القواعد الإلزامية وملخصات الملفات وسؤال المستخدم.
ردك يجب أن يحتوي **فقط** على رقم الملف المختار بالصيغة: `#N`.
...
ملخصات الملفات (استخدمها لتحديد الملف الأنسب):
---
{summaries}
---

الملفات المتوفرة (اختر رقمًا واحدًا فقط من هنا):
---
{doc_list_str}
---

الرد المطلوب (رقم واحد فقط أو #NONE):
"""
    ...
    for attempt in range(SELECTION_MAX_RETRIES):
        try:
            ...
            # MODIFIED: Enforce returning only one index, even if the model returns more.
            if indices_found:
                return [int(indices_found[0])]
...
def get_deepseek_response_stream(prompt: str, api_key: str, chat_history: List[Dict[str, str]], context_doc_count: int) -> Generator[str, None, None]:
    """Gets a streaming response from the API (Unchanged)."""
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
...
def build_prompt(task_description: str, rules: str, context: str, question: Optional[str] = None) -> str:
    """Builds the final prompt for the LLM (Unchanged)."""
    question_section = f"\nسؤال المستخدم:\n---\n{question}\n---\n" if question else ""
    return f"""
...
def main():
    ...
    with chat_container:
        ...
        # --- Action UI: Uploader (Triggers summarization) ---
        if st.session_state.action == "upload_doc":
            ...
            if uploaded_file is not None:
                ...
                # Generate summary for the newly uploaded file
        ...
        # --- Action UI: Summarizer (On-demand) ---
        ...
        # A. Handle new on-demand summary request
        ...
        # B. Handle new question from chat input (REFACTORED with new two-step logic)
        if prompt := st.chat_input("اسأل سؤالاً عن مستنداتك...", disabled=(st.session_state.action is not None)):
            ...
            with st.chat_message("assistant"):
                ...
                # Step 1: Document Selection using Summaries
                if USE_TWO_STEP_SELECTION and st.session_state.doc_names:
                    with st.spinner("جاري تحليل سؤالك وتحديد الوثيقة المناسبة..."):
                        all_summaries = load_all_summaries(SUMMARY_FOLDER_PATH)
                        selected_indices = get_document_selection_with_summaries(
                            prompt, st.session_state.doc_names, st.session_state.rules_content, all_summaries, API_KEY
                        )

                    if selected_indices is None: # Error case
                        error_msg = "حدث خطأ أثناء اختيار الوثيقة. يرجى إعادة المحاولة."
                        st.error(error_msg)
                        st.session_state.messages.append({"role": "assistant", "content": error_msg})
                        st.rerun()
                    
                    elif not selected_indices: # No relevant document found
                        st.info("لم يتم تحديد وثيقة ذات صلة. سأجيب بناءً على القواعد العامة فقط.")
                        final_context_text = ""
                    
                    else: # A single document was selected
                        selected_index = selected_indices[0]
                        selected_name = st.session_state.doc_names[selected_index]
                        context_doc_count = 1
                        
                        review_notice = f"سأراجع الملف التالي للإجابة على سؤالك: `{selected_name}`"
                        st.info(review_notice)
                        st.session_state.messages.append({"role": "assistant", "content": review_notice, "is_review_notice": True})
                        
                        with st.spinner(f"جاري تحميل محتوى الملف '{selected_name}'..."):
                            file_path = os.path.join(FOLDER_PATH, selected_name)
                            _, text = read_file_content(file_path)
                            if text:
                                final_context_text = f"المصدر: {selected_name}\n\nالمحتوى: {text}"
                            else:
                                st.warning(f"لم أتمكن من قراءة محتوى الملف المختار: {selected_name}")
                                final_context_text = ""
                else:
                    # Fallback if selection is off or no docs exist
                    st.info("لم يتم تفعيل اختيار الوثائق أو لا توجد وثائق. سأجيب بناءً على القواعد العامة.")
                    final_context_text = ""

                # Step 2: Generate Final Answer using the selected context
                with st.spinner("جاري توليد الرد..."):
                    ...
                    response_placeholder = st.empty()
                    full_response = ""
                    # Exclude notices from chat history sent to API
                    chat_history_for_api = [{"role": m["role"], "content": m["content"]} for m in st.session_state.messages[:-1] if not m.get("is_review_notice")]
                    
                    stream_generator = get_deepseek_response_stream(final_prompt, API_KEY, chat_history_for_api, context_doc_count)
                    for chunk in stream_generator:
                        full_response += chunk
                        response_placeholder.markdown(full_response + "▌")
                    response_placeholder.markdown(full_response)
    ...
    # --- 3. Footer with Quick Actions (Unchanged) ---
    with st.container():
